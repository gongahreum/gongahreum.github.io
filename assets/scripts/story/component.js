...
if (navigator.maxTouchPoints == 5 && !/Android/i.test(navigator.userAgent))
...
var scrollParent =
  !STORYIPHONE && !STORYANDROID
    ? window
    : document.querySelector("#story-wrap");
...
window.STORY = {
  ...
  load: function load() {
    ...
    document
      .querySelector(".sticky-wrap.sticky1 .message-txt")
      .classList.add("txt-show");
  ...
  scroll: function scroll(callback, _ease) {
    ...
    function esUpdate() {
      ...
      var scrollY =
        !STORYIPHONE && !STORYANDROID
          ? window.pageYOffset
          : document.querySelector("#story-wrap").scrollTop;
  ...
  bodyScroll: function bodyScroll(y) {
    if (STORYLOAD) {
      ...
      document.querySelector(".scroll-induce-wrap").style.opacity =
        1 - y / 1000;
      if (!STORYMOBILE) {
        vestiScrollPercent =
          (y /
            (document.querySelector(".page-all-height").clientHeight -
              STORY.wH)) *
          100;
      } else {
        vestiScrollPercent =
          (y /
            (document.querySelector(".mobile-page-all-height").clientHeight -
              STORY.wH)) *
          100;
      }
      preTarget = y;
  ...
  stickyScroll: function stickyScroll(y) {
    stickyWrap.forEach(function (_this, idx) {
      ...
      _percent = Math.max(
        0,
        Math.min(1, (y - _this.offsetTop) / (_this.clientHeight - STORY.wH))
      );
      _percent2 = Math.max(
        0,
        Math.min(1, (y - _this.offsetTop) / _this.clientHeight)
      );
      if (!_this.classList.contains("cover-sticky")) {
        _move = Math.max(
          -STORY.wH * 2,
          Math.min(_this.clientHeight, y - _this.offsetTop)
        );
      } else {
        _move = Math.max(
          -STORY.wH * 2,
          Math.min(_this.clientHeight - STORY.wH, y - _this.offsetTop)
        );
      }
      stickyCon.style.transform = "translate3d(0," + _move + "px,0)";
      if (_move < 0) _this.classList.remove("show");
      if (!_this.classList.contains("cover-sticky")) {
        if (_move >= 0 && _move < _this.clientHeight)
      ...
      if (
        _this.classList.contains("show") ||
        _this.classList.contains("hide")
      ) {
      ...
      var V3Show = document
        .querySelector(".section-voice.v3")
  ...
  resize: function resize() {
    ...
    resizeSet = setTimeout(function () {
      STORY.wH =
        html.classList.contains("chrome") && STORYMOBILE
      ...
      if (!STORYMOBILE) {
        document.querySelector(".page-all-height").style.height =
          srotyScrollCon.clientHeight + "px";
      } else {
        document.querySelector(".mobile-page-all-height").style.height =
          srotyScrollCon.clientHeight + "px";
      }
  ...
  fakeHeaderShow: function fakeHeaderShow(
    percent,
    start,
    speed,
    pageSpeed,
    ...
    if (!html.classList.contains("ie")) {
      ...
      if (dsmtHeader.querySelector(".fake-header"))
        dsmtHeader.querySelector(".fake-header").style.opacity =
          increaseOpacity;
      if (document.querySelector(".fake-page"))
        document.querySelector(".fake-page").style.opacity = increaseOpacity2;
    }
  },
...
if (!STORYIPHONE && !STORYANDROID) {
  window.addEventListener(
    "scroll",
    STORY.scroll(STORY.bodyScroll, STORY.scrollDuration)
  );
} else {
  document
    .querySelector("#story-wrap")
    .addEventListener(
      "scroll",
      STORY.scroll(STORY.bodyScroll, STORY.scrollDuration)
    );
...
window.addEventListener("resize", function () {
  if (!STORYKAKAOTALK) {
    ...
    var sV2Show = document
      .querySelector(".section-voice.v2")
      .classList.contains("show");
    var sV3Show = document
      .querySelector(".section-voice.v3")
      .classList.contains("show");
    var sV3Hide = document
      .querySelector(".section-voice.v3")
      .classList.contains("hide");
    if (!sV2Show && !sV3Show) {
      if (!sV3Hide) STORY.resize();
    }
    currentScrollOffset = !STORYIPHONE
      ? window.pageYOffset
      : document.querySelector("#story-wrap").scrollTop;
    scrollParent.scrollTo(0, currentScrollOffset + 1);
  }
  ...
  move: function move(e) {
    ...
    if (Math.abs(this.prevY - this.currentY) > 10 && this.moveY < 20) {
      scrollParent.scrollTo(
        0,
        currentScrollOffset + (this.prevY - this.currentY) * 2.5
      );
    } else {
...
window.addEventListener("touchmove", function (e) {
  if (STORYANDROID && !html.classList.contains("scroll-hide"))
    touchAndroid.move(e);
});
...
var setOldAnd;
document
  .querySelector(".voice-mask .text-input")
    ...
    if (!STORYIE && STORYANDROID) {
      setTimeout(function () {
        ...
        if (!STORYOLDANDROID) {
          scrollParent.scrollTo(
            0,
            secV3.offsetTop + secV3.clientHeight - STORY.wH + STORY.wH * 0.28
          );
        } else {
...
document
  ...
  .addEventListener("blur", function (e) {
    ...
    STORY.scrollDuration = !STORYMOBILE
      ? 0.9
      : !STORYKAKAOTALK
      ? !STORYANDROID
        ? 0
  ...
  if (!doesPageDimmed.classList.contains("hide")) {
    $(doesPageDimmed).on("transitionend.trans1", function () {
      ...
      topSetTime1 = setTimeout(function () {
        ...
        document
          .querySelector("#dsmtHeader")
          .removeAttribute("data-color-type");
...
function onYouTubeIframeAPIReady() {
  document
    .querySelectorAll(".youtube-video-layer .video-wrap")
...
var doesVideo = {
  play: function play(_this) {
    ...
    document
      .querySelectorAll(".skip-to-content-link *")
      .forEach(function (_this) {
  ...
  pause: function pause(_this) {
    ...
    var thisLayer = document.querySelector(".youtube-video-layer");
    var idx = _this.getAttribute("data-idx");
    STORY.scrollDuration = !document
      .querySelector(".horizontal-message")
      .classList.contains("show")
      ? 0.985
      : 0.9;
    ...
    document
      ...
      .forEach(function (_this) {
        ...
        if (!_this.classList.contains("aria-hidden"))
    ...
    thisLayer.querySelectorAll(".video-wrap").forEach(function (_this, i) {
      if (i == idx) {
        ...
        videoShowSetTime3 = setTimeout(function () {
          ...
          document
            .querySelectorAll(".button-video-play")
            .forEach(function (_this, i) {
              var mobileHorizon = document
                .querySelector(".horizontal-message")
                .classList.contains("show")
                ? true
                : false;
              ...
              if (i == idx) {
                _this.focus();
                if (
                  !vestibulopathyWrap.classList.contains("active") &&
                  !html.classList.contains("ie")
                )
                  scrollParent.scrollTo({
...
var doesPageBtnEvent = {
  show: function show() {
    if (STORYLOAD) {
      ...
      if (document.querySelector(".fake-page"))
        document.querySelector(".fake-page").classList.add("hide");
    }
  },
  hide: function hide() {
    if (STORYLOAD) {
      ...
      if (document.querySelector(".fake-page"))
        document.querySelector(".fake-page").classList.remove("hide");
    }
  },
  ...
  click: function click(_this, idx) {
    if (STORYLOAD) {
      ...
      if (!vestibulopathyEvent.vestibulopathyRemove)
  ...
  scroll: function scroll(y) {
    doesPageHash.forEach(function (_this, idx) {
      ...
      var _bottom = doesPageHash[idx + 1]
        ? doesPageHash[idx + 1]
        : doesPageHash[idx] + 10000;
...
doesPageBtn.querySelectorAll("a").forEach(function (_this, idx) {
  _this.addEventListener("click", function (e) {
    ...
    if (STORY.wW <= 767) {
      ...
      if (doesPageBtnClick && !vestibulopathyWrap.classList.contains("active"))
        doesPageBtnEvent.click(_this, idx);
    }
  });
  _this.addEventListener("focus", function () {
    if (STORY.wW > 767 && !vestibulopathyWrap.classList.contains("active"))
      doesPageBtnEvent.show();
  });
...
document
  .querySelector(".section-voice .text-input")
    ...
    __this.addEventListener("focus", function () {
      __this.addEventListener("keyup", function (e) {
        if (e.keyCode == 9)
          window.scrollTo(
            0,
            _this.offsetTop + _this.clientHeight - STORY.wH * 1.5
          );
      });
    });
  });
...
window.vestibulopathyEvent = {
  ...
  show: function show() {
    if (!vestibulopathyEvent.vestibulopathyRemove)
      vestibulopathyWrap.style.display = "block";
  },
  hide: function hide() {
    if (!vestibulopathyEvent.vestibulopathyRemove)
      vestibulopathyWrap.style.display = "none";
  ...
  move: function move(idx) {
    ...
    if (STORY.wW > 767) {
      ...
      var _scrollTotalKoEn = !html.classList.contains("en-page")
        ? 92508
        : 92440;
      if (!STORYMOBILE) {
        vestiScrollP =
